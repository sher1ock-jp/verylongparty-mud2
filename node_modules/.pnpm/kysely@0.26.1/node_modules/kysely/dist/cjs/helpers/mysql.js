...
function jsonArrayFrom(expr) {
    return (0, sql_js_1.sql) `(select cast(coalesce(json_arrayagg(json_object(${sql_js_1.sql.join(getJsonObjectArgs(expr.toOperationNode(), 'agg'))})), '[]') as json) from ${expr} as agg)`;
}
...
function jsonObjectFrom(expr) {
    return (0, sql_js_1.sql) `(select json_object(${sql_js_1.sql.join(getJsonObjectArgs(expr.toOperationNode(), 'obj'))}) from ${expr} as obj)`;
}
...
function getJsonObjectArgs(node, table) {
    return node.selections.flatMap(({ selection: s }) => {
        if (reference_node_js_1.ReferenceNode.is(s) && column_node_js_1.ColumnNode.is(s.column)) {
            return [
                sql_js_1.sql.lit(s.column.column.name),
                sql_js_1.sql.id(table, s.column.column.name),
            ];
        }
        else if (column_node_js_1.ColumnNode.is(s)) {
            return [sql_js_1.sql.lit(s.column.name), sql_js_1.sql.id(table, s.column.name)];
        }
        else if (alias_node_js_1.AliasNode.is(s) && identifier_node_js_1.IdentifierNode.is(s.alias)) {
            return [sql_js_1.sql.lit(s.alias.name), sql_js_1.sql.id(table, s.alias.name)];
        }
        else {
