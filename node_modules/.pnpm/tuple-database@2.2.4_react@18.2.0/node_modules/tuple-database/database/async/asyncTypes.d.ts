...
export type AsyncTupleDatabaseClientApi<S extends KeyValuePair = KeyValuePair> = {
    ...
    subspace: <P extends TuplePrefix<S["key"]>>(prefix: P) => AsyncTupleDatabaseClientApi<RemoveTupleValuePairPrefix<S, P>>;
    /** Arguments to transact() are for internal use only. */
    transact: (txId?: TxId, writes?: WriteOps<S>) => AsyncTupleRootTransactionApi<S>;
};
export type AsyncTupleRootTransactionApi<S extends KeyValuePair = KeyValuePair> = {
    ...
    set: <Key extends S["key"]>(tuple: Key, value: ValueForTuple<S, Key>) => AsyncTupleRootTransactionApi<S>;
...
export type AsyncTupleTransactionApi<S extends KeyValuePair = KeyValuePair> = {
    ...
    exists: <T extends S["key"]>(tuple: T) => Promise<boolean>;
    subspace: <P extends TuplePrefix<S["key"]>>(prefix: P) => AsyncTupleTransactionApi<RemoveTupleValuePairPrefix<S, P>>;
    set: <Key extends S["key"]>(tuple: Key, value: ValueForTuple<S, Key>) => AsyncTupleTransactionApi<S>;
...
export type ReadOnlyAsyncTupleDatabaseClientApi<S extends KeyValuePair = KeyValuePair> = {
    scan: <T extends S["key"], P extends TuplePrefix<T>>(args?: ScanArgs<T, P>, txId?: TxId) => Promise<FilterTupleValuePairByPrefix<S, P>[]>;
    get: <T extends S["key"]>(tuple: T, txId?: TxId) => Promise<ValueForTuple<S, T> | undefined>;
    exists: <T extends S["key"]>(tuple: T, txId?: TxId) => Promise<boolean>;
    subspace: <P extends TuplePrefix<S["key"]>>(prefix: P) => ReadOnlyAsyncTupleDatabaseClientApi<RemoveTupleValuePairPrefix<S, P>>;
};
export type AsyncCallback<S extends KeyValuePair = KeyValuePair> = (writes: WriteOps<S>, txId: TxId) => void | Promise<void>;
//# sourceMappingURL=../../../src/database/async/asyncTypes.d.ts.map