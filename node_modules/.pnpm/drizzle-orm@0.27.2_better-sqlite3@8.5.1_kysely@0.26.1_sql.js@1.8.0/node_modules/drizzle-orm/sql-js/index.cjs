...
var session = require('../session-60cba450.cjs');
...
class SQLJsSession extends session.SQLiteSession {
    ...
    transaction(transaction, config = {}) {
        ...
        this.run(alias.sql.raw(`begin${config.behavior ? ` ${config.behavior}` : ''}`));
        try {
            const result = transaction(tx);
            this.run(alias.sql `commit`);
            return result;
        }
        catch (err) {
            this.run(alias.sql `rollback`);
            throw err;
        }
    }
}
...
class SQLJsTransaction extends session.SQLiteTransaction {
    transaction(transaction) {
        ...
        tx.run(alias.sql.raw(`savepoint ${savepointName}`));
        try {
            const result = transaction(tx);
            tx.run(alias.sql.raw(`release savepoint ${savepointName}`));
            return result;
        }
        catch (err) {
            tx.run(alias.sql.raw(`rollback to savepoint ${savepointName}`));
            throw err;
        }
    }
}
...
class PreparedQuery extends session.PreparedQuery {
    ...
    all(placeholderValues) {
        ...
        return rows.map((row) => alias.mapResultRow(fields, row.map((v) => normalizeFieldValue(v)), joinsNotNullableMap));
    }
    get(placeholderValues) {
        ...
        return alias.mapResultRow(fields, row.map((v) => normalizeFieldValue(v)), joinsNotNullableMap);
    }
