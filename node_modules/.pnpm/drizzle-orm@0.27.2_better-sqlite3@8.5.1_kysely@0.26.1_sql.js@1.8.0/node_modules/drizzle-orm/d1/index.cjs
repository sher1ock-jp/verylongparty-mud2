...
class SQLiteD1Session extends session.SQLiteSession {
    ...
    async transaction(transaction, config) {
        ...
        await this.run(alias.sql.raw(`begin${config?.behavior ? ' ' + config.behavior : ''}`));
        try {
            const result = await transaction(tx);
            await this.run(alias.sql `commit`);
            return result;
        }
        catch (err) {
            await this.run(alias.sql `rollback`);
            throw err;
        }
    }
}
...
class D1Transaction extends session.SQLiteTransaction {
    async transaction(transaction) {
        ...
        await this.session.run(alias.sql.raw(`savepoint ${savepointName}`));
        try {
            const result = await transaction(tx);
            await this.session.run(alias.sql.raw(`release savepoint ${savepointName}`));
            return result;
        }
        catch (err) {
            await this.session.run(alias.sql.raw(`rollback to savepoint ${savepointName}`));
            throw err;
        }
    }
}
...
class PreparedQuery extends session.PreparedQuery {
    ...
    async get(placeholderValues) {
        ...
        return alias.mapResultRow(fields, rows[0], joinsNotNullableMap);
    }
